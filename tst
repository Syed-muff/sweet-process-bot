...
from gemini import upload_to_gemini, generate_recommended_questions, generation_config
from utils import allowed_file, find_relevant_page, log_event, dummy_helper
...
def upload_pdf():
    ...
    if file and allowed_file(file.filename):
        ...
        # → Skipping wait_for_files_active per your request
...
def handle_request():
    ...
    if request.json and "user_input" in request.json:
        ...
        pre_prompt = (
            "You are a query assistant. Your task is to answer the given queries strictly "
            "based on the provided document. Respond in plain text, without any formatting. "
        ...
        if filename:
            ...
            if not os.path.exists(uploaded_pdf_path):
                log_event("Uploaded PDF missing; falling back to default.")
                filename = None
        else:
        ...
        if not os.path.exists(pdf_path):
            log_event("PDF not found (uploaded or default).")
        ...
        full_query = [gemini_file, user_input] if gemini_uri else [user_input]
        ...
        try:
            ...
            recommended_questions = generate_recommended_questions(
                user_input,
                model,
                gemini_file.uri if gemini_uri else None
            )
            ...
            pdf_url = f"/uploads/{filename}" if filename else url_for('static', filename='default.pdf')
            log_event(f"PDF URL constructed: {pdf_url}")
